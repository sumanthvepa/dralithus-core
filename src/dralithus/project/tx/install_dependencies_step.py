...
class InstallDependenciesStep(ExecutionStep):
  ...
  def _install_and_snapshot(self) -> None:
    """
      ...
      :return: None
      :raises DralithusProjectError: When a prerequisite is missing
        or installation or snapshotting fails
    """
  ...
  def _write_requirements(
    ...
    """
      ...
      The file is written atomically, via a temporary file in the
      same directory followed by os.replace, so a pre-existing file
      is never left truncated or partially written. Ownership is
      recorded only when the file was absent before this step
      created it.

      :param venv_python: The venv Python interpreter
      :param project_root: The project root directory
      :return: None
      :raises DralithusProjectError: When pip freeze or the write
        fails
    """
    result = self._run(
  ...
  def _verify_regular_file_or_absent(path: Path) -> None:
    """
      ...
      A symlink (valid, dangling, or wrong-type), a directory, or
      any other non-regular entry is rejected: requirements.txt is a
      regenerated artifact, so the step never writes through,
      replaces, or deletes such a thing.
  ...
  def _run(
    ...
    """
      ...
      :raises DralithusProjectError: When the command cannot be run
        or exits non-zero
    """
    try:
